...
namespace UnityRawInput
{
    public static class RawKeyInput
    {
        ...
        public static List<RawKey> KeysToIntercept { get; set; }
        ...
        private static int HandleHookProc (int code, IntPtr wParam, IntPtr lParam)
        {
            ...
            bool interceptThisKey = KeysToIntercept.Contains(key) && InterceptMessages;

            if (isKeyDown) HandleKeyDown(key);
            else HandleKeyUp(key);
            

            return interceptThisKey ? 1 : Win32API.CallNextHookEx(hookPtr, 0, wParam, lParam);
        }
